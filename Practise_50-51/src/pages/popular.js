...
`;
  ...
  const fetchPopularMovies = async () => {
    ...
    currentMovies = response.results.length == 0 ? [] : response.results;
  };
  const loadMovies = () => {
    if (currentMovies.length == 0) {
      content = "<h3>We could not find any movies</h3>";
